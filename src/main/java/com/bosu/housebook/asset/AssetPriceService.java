...
import com.bosu.housebook.asset.vehicle.EncarPriceClient;
...
public class AssetPriceService {
    ...
    public RefreshResult refreshPrices(List<Asset> assets) {
        ...
        for (Asset asset : assets) {
            ...
            if (asset.getType() == AssetType.VEHICLE) {
                if (asset.getEncarUrl() == null || asset.getEncarUrl().isBlank()) {
                    continue;
                }
                Optional<BigDecimal> averagePrice = encarPriceClient.fetchAveragePrice(asset.getEncarUrl());
                if (averagePrice.isEmpty()) {
                    failed++;
                    continue;
                }
                asset.updatePrice(averagePrice.get(), LocalDateTime.now());
                updated++;
                continue;
            }
